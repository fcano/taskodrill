...
class Task(models.Model):
    ...
    @classmethod
    def pull_back_flexible_tasks(cls, user):
        """
        After capacity is freed (e.g. a task marked done), try to move
        pending flexible-due-date tasks to earlier dates without causing
        any non-flexible task to become late.

        Processes one task at a time (latest due_date first),
        recalculating planned dates after each move.
        Stops when no more tasks can be moved earlier.
        """
        holiday_ranges = HolidayPeriod.get_holiday_ranges()
        ...
        for _ in range(50):
            ...
            ).exclude(pk__in=tried_pks).order_by('-due_date').first()

            if flex_task is None:
                break

            original_due_date = flex_task.due_date
            task_length = float(flex_task.length or 1)

            candidate = datetime.date.today() + ONE_DAY
            if not cls.is_working_day(candidate, holiday_ranges):
                candidate = cls.next_business_day(datetime.date.today(), holiday_ranges)

            best_candidate = None
            for _ in range(365):
                if candidate >= original_due_date:
                    break
                total_hours = cls.objects.filter(
                    user=user,
                    status=cls.PENDING,
                    due_date=candidate,
                ).exclude(pk=flex_task.pk).aggregate(
                    total=Sum('length')
                )['total'] or 0
                if float(total_hours) + task_length <= 8:
                    best_candidate = candidate
                    break
                candidate = cls.next_business_day(candidate, holiday_ranges)

            if best_candidate is None:
                tried_pks.add(flex_task.pk)
                continue

            cls.objects.filter(pk=flex_task.pk).update(due_date=best_candidate)
            cls.get_task_plan('nextactions', None, None, None, user)
            ...
            if cls.objects.filter(
                ...
                cls.objects.filter(pk=flex_task.pk).update(due_date=original_due_date)
                cls.get_task_plan('nextactions', None, None, None, user)
                tried_pks.add(flex_task.pk)
                continue
    ...
    def save(self, *args, **kwargs):
        ...
        Task.pull_back_flexible_tasks(self.user)
...
class Goal(models.Model):
    ...
    def update_goal_tasks_due_date(self):
        ...
        if self.due_date:
            ...
            # First assignment date is tomorrow or the next business day
            due_date = datetime.date.today() + datetime.timedelta(1)
            if not Task.is_working_day(due_date, holiday_ranges):
                due_date = Task.next_business_day(due_date, holiday_ranges)

            # Calculate working days from first assignment date to goal due date
            num_working_days = Goal.weekdays_between(due_date, self.due_date, holiday_ranges)

            tasks_to_update = []

            if num_working_days <= 0:
                # No working days left: assign all tasks to goal due date
                for task in tasks:
                    if task.due_date != self.due_date:
                        task.due_date = self.due_date
                        tasks_to_update.append(task)
            elif num_working_days >= len(tasks):
                # More (or equal) working days than tasks: space tasks evenly
                if len(tasks) == 1:
                    working_days_per_task = 0
                else:
                    working_days_per_task = math.floor((num_working_days - 1) / (len(tasks) - 1))
                ...
                # More tasks than working days: distribute extra tasks to first days
                base_tasks_per_day = len(tasks) // num_working_days
                extra_tasks = len(tasks) % num_working_days
                task_idx = 0
                day = 0
                while task_idx < len(tasks):
                    # First 'extra_tasks' days get one additional task
                    count = base_tasks_per_day + (1 if day < extra_tasks else 0)
                    for task in tasks[task_idx:task_idx+count]:
                        if task.due_date != due_date:
                            task.due_date = due_date
                            tasks_to_update.append(task)
                    task_idx += count
